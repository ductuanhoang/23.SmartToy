...
void loop(void)
{
    ...
    for (uint16_t cnt;; cnt++) //int i=0;i<100;i++)
    {
        ...
        if ((cnt & 0x7ff) == 0)
        {
            ...
            if (status.detect)
                DbgPrt("Some key is touched; ");
            if (status.cycle)
                DbgPrt("cycle time overrun; ");
            if (status.error)
                DbgPrt("some key is errored; "); // Expected unless you have hooked up EVERY key
            if (status.change)
                DbgPrt("change pin; ");
            if (status.eeprom)
                DbgPrt("EEPROM error; "); // Expected since we aren't using it (yet).
            if (status.eeprom)
                DbgPrt("device just reset; ");
            if (status.guard)
                DbgPrt("guard is detecting; ");
            DbgPrt("\n");
            ret = qt.getAllErrors();
