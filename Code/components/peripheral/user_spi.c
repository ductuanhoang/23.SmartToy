...
#define TOUCH_2_PIN_NUM_MISO 12
#define TOUCH_2_PIN_NUM_MOSI 23
#define TOUCH_2_PIN_NUM_CLK 14
 ...
 ***********************************************************************************************************************/
static spi_device_handle_t touch1_spi; // spi interface touch 1
static spi_device_handle_t touch2_spi; // spi interface touch 2
...
void user_spi_init(uint8_t spi_number)
{
    if (spi_number == 1)
    {
        ...
        spi_device_interface_config_t devcfg = {
            ...
            .mode = 3,                           // SPI mode 3
            .spics_io_num = TOUCH_1_PIN_NUM_CS,  // CS pin
        ...
        ret = spi_bus_add_device(TOUCH_1_SPI_NUM, &devcfg, &touch1_spi);
        assert(ret == ESP_OK);
    ...
    else if (spi_number == 2)
    {
        ...
        esp_err_t ret;
        spi_bus_config_t buscfg = {
            .miso_io_num = TOUCH_2_PIN_NUM_MISO,
            .mosi_io_num = TOUCH_2_PIN_NUM_MOSI,
            .sclk_io_num = TOUCH_2_PIN_NUM_CLK,
            .quadwp_io_num = -1,
            .quadhd_io_num = -1,
        };
        ...
        spi_device_interface_config_t devcfg = {
            ...
            .clock_speed_hz = 1 * 100 * 1000,    // Clock out at 1 MHz
            .mode = 3,                           // SPI mode 3
            .spics_io_num = TOUCH_2_PIN_NUM_CS,  // CS pin
            .queue_size = 20,                    // We want to be able to queue 7 transactions at a time
            .pre_cb = spi_pre_transfer_callback, // Specify pre-transfer callback to handle D/C line
            .post_cb = spi_post_transfer_callback,
            .input_delay_ns = EEPROM_INPUT_DELAY_NS, // the EEPROM output the data half a SPI clock behind.
        ...
        // Initialize the SPI bus
        ret = spi_bus_initialize(TOUCH_2_SPI_NUM, &buscfg, 0);
        assert(ret == ESP_OK);

        // Attach the touch to the SPI bus
        ret = spi_bus_add_device(TOUCH_2_SPI_NUM, &devcfg, &touch2_spi);
        assert(ret == ESP_OK);
...
/***********************************************************************************************************************
 * Function Name: spi_touch2_cs_pause
 * Description  : pause communication touch 2
 * Arguments    : none
 * Return Value : none
 ***********************************************************************************************************************/
...
/***********************************************************************************************************************
 * Function Name: spi_touch2_cs_resume
 * Description  : resume communication touch 2
 * Arguments    : none
 * Return Value : none
 ***********************************************************************************************************************/
...
void spi_touch1_write(uint8_t value)
{
    ...
    ret = spi_device_transmit(touch1_spi, &t);
...
esp_err_t spi_touch1_read(uint8_t regAddr, uint8_t* data, uint8_t length)
{
    ...
    esp_err_t err = spi_device_polling_transmit(touch1_spi, &trans);
    if (err!= ESP_OK)
    {
...
void spi_touch2_write(uint8_t value)
{
    ...
    t.tx_buffer = &value;     // The data is the cmd itself
    t.user = (void *)0;       // D/C needs to be set to 0
    ret = spi_device_transmit(touch2_spi, &t);
    assert(ret == ESP_OK); // Should have had no issues.
    // return true;
}
/***********************************************************************************************************************
...
esp_err_t spi_touch2_read(uint8_t regAddr, uint8_t* data, uint8_t length)
{
    ...
    esp_err_t err = spi_device_polling_transmit(touch2_spi, &trans);
    if (err!= ESP_OK)
    {
        printf("read data error\r\n");
        return err;
